...
const ServiceList = () => {
  ...
  const getDateCategory = (date: Date) => {
    ...
    const diffTimeDurrationObject = dateInDateTime.diffNow(["year", "month", "week", "day", "hour"]);


    const endOfToday = DateTime.now().set({ hour: 23, minute: 59, second: 59 });
    const diffTimeFromNowToEOD = DateTime.now().diff(endOfToday, ["year", "month", "week", "day", "hour"]);


    if (diffTimeDurrationObject.days === 0) { // Today
      return "Today";
    }
    if (diffTimeDurrationObject.days == -1) { // Yesterday
      return;
    }

    if (diffTimeDurrationObject.weeks === -1) { // Last week
      return `Week ago`;
    }
    if (diffTimeDurrationObject.months === -1) { // Last week
      return `Month ago`;
    }
    if (diffTimeDurrationObject.years === -1) { // Last week
      return `Year ago`;
    }

    const categories: { label: string; items: Transaction[] }[] = [
      { label: "Today", items: [] },
      { label: "Yesterday", items: [] },
      { label: "Week ago Week", items: [] },
      { label: "Month ago", items: [] },
      { label: "Year ago", items: [] },
      { label: "Older", items: [] }
    ];

    // Group transactions by category
    transactions.forEach(transaction => {
      const category = getDateCategory(new Date(transaction.createdAt));
      categories.find(c => c.label === category)?.items.push(transaction);
    });
